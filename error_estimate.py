import pickle
import numpy as np
from base import read_raw_data, mask_points, extract_fit_part, read_database, union
...
def get_fe2_re(fit_res, wave, flux, error):
    ...
    for each in FeII_template_obs.center_l1:
        list.append(each - 2.0 * width_l1 * each, each + 2.0 * width_l1 * each)
    for each in FeII_template_obs.center_n3:
        list.append(each - 2.0 * width_n3 * each, each + 2.0 * width_n3 * each)
    total_line = union(line_list)
    re = 0.0
    for each in total_line:
        temp = get_error(np.mean(each), (each[1] - each[0]) * 0.25)
        re = re + temp ** 2.0
    return np.sqrt(re)
...
def estimate_error_single(rmid, hbetab_dic, hbetan_dic, o3_dic, fe2_dic, mjd):
    ...
    res = read_database(Location.project_loca + "result/fit_with_temp/data/" +
